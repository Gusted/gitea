	...
	commitReadingLoop:
		for {
			...
			case "tree":
				var n int64
				for n < size {
					mode, fname, fileHash, count, err := git.ParseTreeLine(batchReader, shaLen, modeBuf, fnameBuf, workingShaBuf)
					if err != nil {
						return nil, err
					}
					n += int64(count)
					if bytes.Equal(fileHash, hash[:]) {
						...
						git.To40ByteSHA(fileHash, sha40Byte)
